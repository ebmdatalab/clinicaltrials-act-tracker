...
def _pluralise(message, number):
    if number != 1:
        message = message.replace("trial", "trials").replace("its", "their")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if data["overdue_today"] or data["late_today"] or data["on_time_today"]:
            message = "Since our last update, "
            if data["overdue_today"]:
                message += _pluralise("{} trial became overdue", data["overdue_today"])
            if data["late_today"]:
                if data["overdue_today"]:
                    message += ", and "
                else:
                    message += " "
                message += _pluralise("{} trial reported late", data["late_today"])
            message += ". "
            if data["on_time_today"]:
                message += _pluralise(
                    "{} trial reported its results on time. ", data["on_time_today"]
                )
            percentage = round(data["reported"] / data["due"] * 100)
            message += "{}% of all due trials " "have reported their results.".format(
                percentage
            )
