...
def _compute_ranks():
    sql = (
        "WITH ranked AS (SELECT date, ranking.id, RANK() OVER ("
        "  PARTITION BY date "
...
def set_current_rankings():
    ...
    with transaction.atomic():
        for sponsor in Sponsor.objects.all():
            due = Trial.objects.due().filter(sponsor=sponsor).count()
            reported = Trial.objects.reported().filter(sponsor=sponsor).count()
            reported_late = (
                Trial.objects.reported_late().filter(sponsor=sponsor).count()
            )
            reported_on_time = (
                Trial.objects.reported_on_time().filter(sponsor=sponsor).count()
            )
            overdue = Trial.objects.overdue().filter(sponsor=sponsor).count()
            ...
            finable_days_late = sponsor.trial_set.visible().aggregate(
                total_finable_days_late=Sum("finable_days_late")
            )["total_finable_days_late"]
            d = {
                "due": due,
                "reported": reported,
                "total": total,
            ...
            ranking = sponsor.rankings.filter(date=sponsor.updated_date)
            if len(ranking) == 0:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        zombies = Trial.objects.filter(updated_date__lt=today).exclude(
            status=Trial.STATUS_NO_LONGER_ACT
        )
        logger.info("Marking %s zombie trials", zombies.count())
        zombies.update(status=Trial.STATUS_NO_LONGER_ACT, updated_date=today)
