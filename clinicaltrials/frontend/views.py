...
def current_count(field_name, date):
    return Ranking.objects.filter(date=date).aggregate(Sum(field_name))[
        field_name + "__sum"
    ]
...
def rankings(request):
    context = {"title": "Who’s sharing their clinical trial results?"}
...
def sponsor(request, slug):
    ...
    days_late = sponsor.trial_set.aggregate(days_late=Sum("finable_days_late"))[
        "days_late"
    ]
    if days_late:
...
def trial(request, registry_id=None):
    ...
    if os.path.exists(annotation):
        with open(annotation, "r") as f:
            ...
            annotation_html += "<p><a href='/page/trials/{}'>Read more...</a></p>".format(
                registry_id
            )
    else:
        annotation_html = ""
