...
class TweetTodayTestCase(TestCase):
    ...
    def test_all_variables(self, twitter_mock):
        ...
        #
        #api_update = MagicMock('api_update')
        #api.PostUpdate.return_value = api_update
        twitter_mock.Api.return_value = api
        out = StringIO()
        call_command('tweet_today', stdout=out)
        api.PostUpdate.assert_called_with('Since our last update, 1 trial became overdue, and 3 trials reported late. 1 trial reported its results on time. 80% of all due trials have reported their results.')
    ...
    def test_single_variable(self, twitter_mock):
        ...
        out = StringIO()
        call_command('tweet_today', stdout=out)
        api.PostUpdate.assert_called_with('Since our last update, 1 trial became overdue. 0% of all due trials have reported their results.')
