...
def mock_ccgov_results(fixture_id):
    fixture_path = os.path.join(
        settings.BASE_DIR, "frontend/tests/fixtures/{}.html".format(fixture_id)
    )
...
class CommandsTestCase(TestCase):
    ...
    def test_import(self, datetime_mock):
        ...
        sample_csv = os.path.join(
            settings.BASE_DIR, "frontend/tests/fixtures/sample_bq.csv"
        )
        ...
        overdue_evenwithexemption = Trial.objects.get(
            registry_id="overdue_evenwithexemption"
        )
        self.assertEqual(overdue_evenwithexemption.status, "overdue")
        self.assertEqual(overdue_evenwithexemption.days_late, 31)

        overdue_withexemption_reported = Trial.objects.get(
            registry_id="overdue_withexemption_reported"
        )
        ...
        overdue_withexemption_late = Trial.objects.get(
            registry_id="overdue_withexemption_late"
        )
    ...
    def test_second_import_with_disappeared_trials(self, datetime_mock):
        ...
        # Import empty file
        sample_csv = os.path.join(
            settings.BASE_DIR, "frontend/tests/fixtures/sample_bq_empty.csv"
        )
        opts = {"input_csv": sample_csv}
        call_command("process_data", **opts)
    ...
    def test_qa(self, datetime_mock):
        ...
        args = []
        sample_csv = os.path.join(
            settings.BASE_DIR, "frontend/tests/fixtures/sample_bq_qa.csv"
        )
        opts = {"input_csv": sample_csv}
        call_command("process_data", *args, **opts)
        ...
        overdueinqa_uncancelled = Trial.objects.get(
            registry_id="overdueinqa_uncancelled"
        )
        ...
        qa = Trial.objects.get(
            registry_id="overdueinqa_manycancelled"
        ).trialqa_set.all()
        ...
        qa = Trial.objects.get(
            registry_id="overdueinqa_cancelled_after_returned"
        ).trialqa_set.all()
        self.assertEqual(len(qa), 3)
    ...
    def test_import_twice(self, datetime_mock, requests_mock):
        ...
        qa = Trial.objects.get(registry_id="overdueinqa_two_months").trialqa_set.all()
        self.assertEqual(len(qa), 1)
