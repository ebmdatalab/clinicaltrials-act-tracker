...
class FrontendTestCase(TestCase):
    ...
    def test_trial_annotation(self, mock_path):
        test_page = os.path.join(
            settings.BASE_DIR, "frontend/tests/fixtures/static_page.md"
        )
...
class ApiResultsTestCase(TestCase):
    ...
    def test_trial_filter(self):
        client = APIClient()
        response = client.get(
            "/api/trials/", {"has_results": True}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 1)
        self.assertEqual(response["results"][0]["title"], self.reported_trial.title)

        response = client.get(
            "/api/trials/", {"status": "overdue"}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 1)
        self.assertEqual(response["results"][0]["title"], self.due_trial.title)

        response = client.get(
            "/api/trials/", {"status": "overdue-cancelled"}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 0)

        response = client.get(
            "/api/trials/", {"search[value]": "overdue-cancelled"}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 0)

        response = client.get(
            "/api/trials/", {"search[value]": "Trial"}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 2)
    ...
    def test_trial_today_filters(self):
        client = APIClient()
        response = client.get(
            "/api/trials/", {"is_overdue_today": "2"}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 1)
        self.assertEqual(response["results"][0]["title"], self.due_trial.title)

        response = client.get(
            "/api/trials/", {"is_no_longer_overdue_today": "2"}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 1)
        self.assertEqual(response["results"][0]["title"], self.reported_trial.title)
    ...
    def test_sponsor_filter(self):
        client = APIClient()
        response = client.get(
            "/api/sponsors/", {"num_trials_min": 0}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 1)
        self.assertEqual(response["results"][0]["slug"], self.sponsor.slug)

        response = client.get(
            "/api/sponsors/", {"num_trials_min": 4}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 0)
    ...
    def test_ranking_filter(self):
        client = APIClient()
        response = client.get(
            "/api/rankings/", {"percentage__gte": 50}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 1)
        self.assertEqual(response["results"][0]["sponsor_slug"], self.sponsor.slug)

        response = client.get(
            "/api/rankings/", {"percentage__lte": 49}, format="json"
        ).json()
        self.assertEqual(response["recordsFiltered"], 0)
...
class ApiPerformanceResultsTestCase(TestCase):
    ...
    def test_performance_single_sponsor(self):
        client = APIClient()
        response = client.get(
            "/api/performance/?sponsor={}".format(self.sponsor1.slug), format="json"
        ).json()
        self.assertEqual(
            response,
            {
    ...
    def test_performance_no_results(self):
        ...
        client = APIClient()
        response = client.get(
            "/api/performance/", {"sponsor": "xyz"}, format="json"
        ).json()
        self.assertEqual(
            response,
            {
